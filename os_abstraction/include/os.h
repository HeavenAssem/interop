...
enum walk_policy_e : uint8_t { NonRecursive, Recursive };

/**
 * @brief: lists all items by path
 * @param: path: path where to look for items
 * @param: extension: file extension preceded by dot (".so", ".dll") [optional, default: empty
 * string]
...
lazy_sequence_t<std::string> walk(const std::string_view & path,
                                  ...
                                  walk_policy_e = NonRecursive, entry_type_e = File);
